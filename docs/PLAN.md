...
- **A second provider: Claude Code, for subscription auth.** `llm.provider: claude_code` reaches Claude through the logged-in Claude Code CLI instead of an API key, so the tool runs on a Claude Code subscription with nothing in `.env`. The adapter (`llm/claude_code.py`) has to reconcile two opposed shapes: `LLMClient` promises a single-shot completion that executes nothing, while Claude Code is an agent that owns its loop and runs tools itself. Forced tool calls map cleanly onto the SDK's `output_format`, so structured output needed no prompt coaching. Tool calls do not map: the framework must keep executing its own tools or the allow-list, audit log, and workspace sandbox stop meaning anything, so tools are registered for their schemas and then every invocation is denied at the permission gate and handed back to the caller. Claude Code's own toolset and settings-file loading are both disabled — the first would route around `tools/`, the second would make a run depend on which directory it started in.
- **Subscription spend is recorded but does not charge the budget.** Claude Code reports what its tokens would have cost through the API. Charging that against `budget.usd_per_project` would stop a run over money nobody was billed, so `ClaudeCodeLLMClient.budget_usd` returns zero unless `llm.subscription_cost_is_billed` says otherwise; the figure still reaches the ledger. This makes `budget.tokens_per_phase` the ceiling that actually bites, which in turn forced the token count to include cache traffic — Claude Code re-sends a ~20k-token harness prompt every call and reports it as cache creation, so counting `input_tokens` alone had a real run recording 2 input tokens instead of ~19,000.
